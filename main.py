#!/usr/bin/env python3
# ============================================
# 🚀 Yasha Group Bot v5.0 — Premium Emoji Edition
# ============================================
...
def build_emoji_entity(emoji_id: str, offset: int = 0, length: int = 2) -> MessageEntity:
    """ساخت یک entity برای ایموجی پریمیوم"""
...
def get_premium_prefix() -> tuple[str, list]:
    """
    ساخت پیشوند با چند ایموجی پریمیوم
    برمیگرداند: (text, entities)
    """
    emoji_list = [
        CUSTOM_EMOJIS["grin"],
...
async def send_premium(chat_id, text: str, context: ContextTypes.DEFAULT_TYPE,
    ...
    """ارسال پیام با پیشوند ایموجی پریمیوم"""
    prefix, prefix_entities = get_premium_prefix()
    full_text = prefix + text
    
    # محاسبه offset برای entities متن اصلی
...
async def reply_premium(update: Update, text: str, context: ContextTypes.DEFAULT_TYPE,
    ...
    """reply با پیشوند ایموجی پریمیوم"""
    prefix, prefix_entities = get_premium_prefix()
    full_text = prefix + text
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        await reply_premium(update, "❌ فقط ادمین‌ها می‌توانند تنظیمات را تغییر دهند!", context)
        return
    await reply_premium(
        update,
