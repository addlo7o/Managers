...
# ذخیره لیست گروه‌ها برای پیام خودکار
active_chats = set()
...
WELCOME_JOKES = [
    ...
    # Welcome, sweet mystery ✨\nReveal yourself slowly to us.

    "Un papillon s'est posé ici 🦋\nReste, la beauté t'appartient.",
    # A butterfly has landed here 🦋\nStay, beauty belongs to you.

    "Bienvenue, dernier rayon du soir 🌅\nRéchauffe nos cœurs avant la nuit.",
    # Welcome, last ray of evening 🌅\nWarm our hearts before the night.
]
...
async def send_emoji_with_text(chat_id, context, emoji_number, text, parse_mode=ParseMode.MARKDOWN):
    """ارسال ایموجی پریمیوم + متن - هر دو حذف خودکار"""
...
# ============================================
# پیام خودکار هر نیم ساعت (با asyncio - بدون JobQueue و دیتابیس)
# ============================================

async def auto_message_loop(app: Application):
    """حلقه پیام خودکار: هر ۳۰ دقیقه به همه گروه‌های فعال پیام میده"""
    await asyncio.sleep(10)  # ۱۰ ثانیه صبر بعد از استارت
    while True:
        try:
            if active_chats:
                ...
                for chat_id in list(active_chats):
                    try:
                        ...
                        await send_emoji_with_text(chat_id, app.bot, emoji_num, AUTO_MESSAGE_TEXT)
                        await asyncio.sleep(2)  # فاصله ۲ ثانیه بین هر گروه برای جلوگیری از rate limit
                    except Exception as e:
                        logger.error(f"Failed to send auto message to {chat_id}: {e}")
                        # اگه ربات از گروه حذف شده باشه، از لیست حذفش کن
                        if "chat not found" in str(e).lower() or "bot was kicked" in str(e).lower():
                            active_chats.discard(chat_id)
            else:
                logger.info("No active chats for auto message. Waiting...")
        except Exception as e:
            logger.error(f"Auto message loop error: {e}")
        
        await asyncio.sleep(1800)  # ۳۰ دقیقه صبر کن
...
async def welcome_member(update, context):
    ...
    # اضافه کردن گروه به لیست active_chats
...
def main():
    ...
    # ========================================
    # راه‌اندازی حلقه پیام خودکار با asyncio
    # ========================================
    ...
    print("🤖 ربات یاشا آماده‌ست! (همه پیام‌ها بعد ۵۰ ثانیه حذف میشن | پیام خودکار هر ۳۰ دقیقه با asyncio)")
