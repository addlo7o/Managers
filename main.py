...
COMMAND_EXPLANATIONS = {
    ...
    def execute(chat_id, user_id, reason):
        ...
        # ذخیره دلیل در لاگ
        log_action(f"User {user_id} banned. Reason: {reason}")
    ...
    def execute(chat_id, user_id, duration=None):
        # تنظیم مجوزها برای عدم ارسال پیام
        permissions = ChatPermissions(can_send_messages=False)
    ...
    def execute(chat_id, user_id):
        # اخراج و بلافاصله آنبن برای امکان بازگشت
        bot.ban_chat_member(chat_id, user_id)
        bot.unban_chat_member(chat_id, user_id)
