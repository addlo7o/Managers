...
async def send_premium_emoji(chat_id, context, number, reply_to=None):
    ...
    entities = [MessageEntity(type=MessageEntity.CUSTOM_EMOJI, offset=0, length=len(text), custom_emoji_id=emoji_id)]
    try:
...
def main():
    ...
    logger.info("ربات یاشا با ایموجی پریمیوم شروع به کار کرد!")
    app.run_polling()
