...
async def auto_delete_message(msg, delay=15):
...
async def send_premium_emoji(chat_id, context, number, reply_to=None):
    """ارسال ایموجی پریمیوم و حذف خودکار بعد از ۱۵ ثانیه"""
    ...
    try:
        ...
        await auto_delete_message(msg)  # حذف بعد ۱۵ ثانیه
        return msg
...
async def reply_msg(update, text, context, reply_markup=None, parse_mode=ParseMode.MARKDOWN):
    """ارسال پاسخ و حذف خودکار بعد از ۱۵ ثانیه"""
    try:
...
async def send_and_delete(context, chat_id, text, parse_mode=ParseMode.MARKDOWN, reply_markup=None):
    """ارسال پیام به گروه و حذف خودکار بعد از ۱۵ ثانیه"""
    try:
...
async def check_sub_callback(update, context):
    ...
    if await is_force_subscribed(query.from_user.id, context):
        ...
        await auto_delete_message(query.message)  # حذف بعد ۱۵ ثانیه
    else:
...
async def welcome_member(update, context):
    ...
    for member in update.message.new_chat_members:
        ...
        await send_premium_emoji(chat_id, context, random_emoji_num)  # این ایموجی هم حذف میشه
        ...
        try:
            ...
            await auto_delete_message(msg)  # حذف ولکام بعد ۱۵ ثانیه
        except:
            pass
...
async def goodbye_member(update, context):
    ...
    try:
        msg = await context.bot.send_message(chat_id=chat_id, text=text, parse_mode=ParseMode.MARKDOWN)
        await auto_delete_message(msg)  # حذف گودبای بعد ۱۵ ثانیه
    except:
        pass
...
def main():
    ...
    print("🤖 ربات یاشا آماده‌ست! (همه پیام‌ها بعد ۱۵ ثانیه حذف میشن)")
