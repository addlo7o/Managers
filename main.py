...
# ============================================
# Real Python Code for Each Command (Dark Theme)
# ============================================
...
async def send_code_explanation(update, context, command, title, code, benefit):
    text = f"📚 **توضیحات فنی دستور: {title}**\n\n━━━━━━━━━━━━━━━━━━━━━━\n📝 **کد پایتون (Dark Mode):**\n\n```python\n{code}\n```\n\n━━━━━━━━━━━━━━━━━━━━━━\n{benefit}\n\n💡 این کد واقعی و کامل است"
...
async def lock_cmd(update, context):
    ...
    try:
        ...
        await send_code_explanation(update, context, "lock", "🔒 قفل گروه", LOCK_PYTHON_CODE, "🛡️ جلوگیری از هرج و مرج در گروه")
    except Exception as e:
...
async def unlock_cmd(update, context):
    ...
    try:
        ...
        await send_code_explanation(update, context, "unlock", "🔓 باز کردن گروه", UNLOCK_PYTHON_CODE, "🎉 بازگشت به حالت عادی گروه")
    except Exception as e:
        await reply_premium(update, f"❌ خطا: {e}", context)
...
async def ban_cmd(update, context):
    ...
    try:
        ...
        await send_code_explanation(update, context, "ban", "🚫 بن کردن کاربر", BAN_PYTHON_CODE, "⚠️ حذف کاربران مخرب از گروه")
    except Exception as e:
        await reply_premium(update, f"❌ خطا: {e}", context)
...
async def mute_cmd(update, context):
    ...
    try:
        if duration:
            ...
            await context.bot.restrict_chat_member(update.effective_chat.id, target.id, permissions=ChatPermissions(can_send_messages=False))
            await reply_premium(update, f"🔇 **{target.first_name}** میوت دائمی شد!", context)
        await send_code_explanation(update, context, "mute", "🔇 میوت کاربر", MUTE_PYTHON_CODE, "⏰ تنبیه موقت کاربر بدون حذف از گروه")
    except Exception as e:
        await reply_premium(update, f"❌ خطا: {e}", context)
...
async def kick_cmd(update, context):
    ...
    try:
        ...
        await send_code_explanation(update, context, "kick", "👢 کیک کاربر", KICK_PYTHON_CODE, "🚪 اخطار جدی بدون حذف دائمی")
    except Exception as e:
        await reply_premium(update, f"❌ خطا: {e}", context)
...
async def purge_cmd(update, context):
    ...
    await send_code_explanation(update, context, "purge", "🧹 پاکسازی پیام‌ها", PURGE_PYTHON_CODE, "🧽 نظافت گروه و حذف پیام‌های اسپم")
...
async def settings_cmd(update, context):
    ...
    await send_code_explanation(update, context, "settings", "⚙️ تنظیمات گروه", SETTINGS_PYTHON_CODE, "🎛️ شخصی‌سازی رفتار ربات")
