...
# ============================================
# ایموجی‌های پریمیوم با IDهای صحیح
# ============================================
...
async def send_premium_emoji(chat_id, context, number, reply_to=None):
    """ارسال ایموجی پریمیوم با طول صحیح"""
    ...
    try:
        return await context.bot.send_message(chat_id=chat_id, text=text, entities=entities, reply_to_message_id=reply_to)
    except Exception as e:
        logger.error(f"Error sending premium emoji {number}: {e}")
        return None

def get_random_emoji():
    """انتخاب یک ایموجی پریمیوم تصادفی"""
...
# ============================================
# تابع حذف پیام بعد از ۱۵ ثانیه (برای جواب دستورات)
# ============================================

async def auto_delete_reply(update, text, context, reply_markup=None, parse_mode=ParseMode.MARKDOWN, delay=15):
    """ارسال پاسخ و حذف آن بعد از delay ثانیه"""
    try:
        ...
        logger.error(f"Auto-delete reply error: {e}")
        return None
...
# ============================================
# ولکام و خداحافظی (با ایموجی تصادفی و شوخی)
# ============================================
...
async def welcome_member(update, context):
    ...
    for member in update.message.new_chat_members:
        ...
        # انتخاب ایموجی پریمیوم تصادفی از بین همه ۱۸ تای موجود
...
# ============================================
# دستورات مدیریتی (همه با حذف خودکار بعد از ۱۵ ثانیه)
# ============================================

async def lock_cmd(update, context):
    if not await is_admin(update, context):
        await auto_delete_reply(update, "❌ فقط ادمین‌ها!", context)
        return
    try:
        ...
        await auto_delete_reply(update, "🔒 گروه قفل شد! فقط ادمین‌ها می‌تونن پیام بدن.", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def unlock_cmd(update, context):
    ...
    try:
        ...
        await send_premium_emoji(update.effective_chat.id, context, 12)
        await auto_delete_reply(update, "🔓 گروه باز شد! همه می‌تونن پیام بدن.", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def ban_cmd(update, context):
    ...
    if not target:
        await auto_delete_reply(update, "❌ روی پیام کاربر reply کن", context)
        return
    try:
        await context.bot.ban_chat_member(update.effective_chat.id, target.id)
        await send_premium_emoji(update.effective_chat.id, context, 7)
        await auto_delete_reply(update, f"🚫 **{target.first_name}** بن شد!", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def unban_cmd(update, context):
    ...
    try:
        ...
        await send_premium_emoji(update.effective_chat.id, context, 12)
        await auto_delete_reply(update, f"✅ **{target.first_name}** آنبن شد!", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def mute_cmd(update, context):
    ...
    target = get_target_user(update)
    if not target:
        await auto_delete_reply(update, "❌ روی پیام کاربر reply کن\nمثال: /mute 1h", context)
        return
    ...
    try:
        if duration:
            ...
            if match:
                ...
                await auto_delete_reply(update, f"🔇 **{target.first_name}** میوت شد برای {duration}!", context)
            else:
                ...
                await auto_delete_reply(update, f"🔇 **{target.first_name}** میوت دائمی شد!", context)
        else:
            await context.bot.restrict_chat_member(update.effective_chat.id, target.id, permissions=ChatPermissions(can_send_messages=False))
            await auto_delete_reply(update, f"🔇 **{target.first_name}** میوت دائمی شد!", context)
    except Exception as e:
...
async def unmute_cmd(update, context):
    ...
    try:
        ...
        await send_premium_emoji(update.effective_chat.id, context, 12)
        await auto_delete_reply(update, f"🔊 **{target.first_name}** آنمیوت شد!", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def kick_cmd(update, context):
    ...
    try:
        ...
        await context.bot.unban_chat_member(update.effective_chat.id, target.id)
        await send_premium_emoji(update.effective_chat.id, context, 7)
        await auto_delete_reply(update, f"👢 **{target.first_name}** کیک شد!", context)
    except Exception as e:
        await auto_delete_reply(update, f"❌ خطا: {e}", context)
...
async def purge_cmd(update, context):
    ...
    deleted = 0
    await auto_delete_reply(update, "🧹 در حال پاکسازی...", context)
    ...
    await auto_delete_reply(update, f"✅ پاکسازی تموم شد! {deleted} پیام حذف شد.", context)
...
async def purge_user_cmd(update, context):
    ...
    for msg_id in range(current, max(current - 200, 1), -1):
        try:
            ...
            pass
        await asyncio.sleep(0.05)
    await auto_delete_reply(update, f"✅ {deleted} پیام از {target.first_name} حذف شد.", context)
